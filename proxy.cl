  ...
  (let* ((request-body (get-request-body req))
	...
	(progn
	  ...
	  (let (protocol response commment header-start given-content-length
	    ...
	    (parse-header-block outbuf 0 outend)
	    ...
	    (setq given-content-length
	      (header-buffer-value outbuf :content-length))
    
    
	    (if* (not (eq (request-method req) :head)
		      (or (<= 100 response 199) 
			  (eq response 204)
			  (eq response 304)))
		    ...
		    (if* (null given-content-length)
		       then (warn "no content length, read till eof")
			    (multiple-value-setq (body-buffers body-length)
			      ...
			      (read-into-block-buffers sock content-length))
	    ...
	    (if-debug-action 
	     :xmit
	     (format *debug-stream* "proxy covnerted headers toward client~%")
	     (dotimes (i cliend)
	    ...
	    (let ((rsock (request-socket req)))
	      ...
	      (write-body-buffers rsock body-buffers body-length)
...
(defun parse-response-buffer (buff end)
  ...
  ;;
  (let (protocol response-code comment)
    ;; 
...
(defun read-into-block-buffers (sock size)
  ...
  (if* (eql size 0)
     then (return-from read-into-block-buffers nil))
  
  (let (res block len bytesleft (bytesread 0))
    ...
    (loop
      (let ((res (read-sequence block sock 
				:start start
				:end (min end (+ start bytesleft)))))
	(if* (<= res start)
	   then ; end of file
		(return)
	   else ; read something
		(if* size 
		   then (decf bytesleft (- res start))
		   else (incf bytesread (- res start)))
		(if* (<= bytesleft 0)
