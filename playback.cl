...
;;
;;
;; $Id: playback.cl,v 1.2 2008/02/04 19:42:27 jkf Exp $
  ...
  (macrolet ((qval (tag) `(cdr (assoc ,tag form :test #'equal))))
    (let ((method (qval :method))
	 ...
	 then ; must do the hack
		...
		(if* ok then (format t "new call id is ~s~%" call-id))
		...
		(multiple-value-bind (ok whole user-id)
		  ...
		  (if* ok then (format t "new user id is ~s~%" user-id))
